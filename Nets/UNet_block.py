...
class UNet_block(nn.Module):
    ...
    def __init__(
        ...
        print("Original UNet but with avg pool and add to channel overlap instead of convolution in residual unit")
    ...
    def _get_up_layer(self, in_channels: int, out_channels: int, strides: int, is_top: bool) -> nn.Module:
        ...
        # now upsample and 2 res subunits to remove grid artifacts
        upsample = torch.nn.Upsample(scale_factor=strides)
        ...
        conv_out = Convolution(
            ...
            dropout=self.dropout,
            bias=self.bias,
            conv_only=(is_top and not self.last_layer_adn),
            is_transposed=False,
            adn_ordering=self.adn_ordering,
        )
        conv = nn.Sequential(upsample, conv_out) 
