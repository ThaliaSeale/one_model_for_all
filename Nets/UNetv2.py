...
class UNetv2(nn.Module):
    ...
    def __init__(
        ...
        print("now two subunits per residual unit in the upsample path, and modified final layer so upsample and two subunits of residual unit keep channels high then add a final convolution at the end reducing channels to one channel at output")
        print("Also using avg pool residual units")
        self.last_layer_adn = last_layer_adn
    ...
    def _get_up_layer(self, in_channels: int, out_channels: int, strides: int, is_top: bool) -> nn.Module:
        ...
        if not is_top:
            ...
            # if it is the top layer, upsample maintaining high number of channels then res unit then final low channel output conv
            conv = Convolution(
                self.dimensions,
                in_channels,
                in_channels,
            ...
            output_conv = Convolution(
                ...
                dropout=self.dropout,
                bias=self.bias,
                conv_only= not self.last_layer_adn,
                is_transposed=False,
                adn_ordering=self.adn_ordering,
            )

            conv = nn.Sequential(conv, ru, output_conv)
