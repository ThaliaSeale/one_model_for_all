...
def extract_features(input_data,layers,pretrained_predictor):
    out = pretrained_predictor(input_data)
    pretrained_model_features = list()
    for layer in layers:
        pretrained_model_features.append(features[layer])
    return pretrained_model_features

def sliding_window_inference_progressive(
    inputs: torch.Tensor | MetaTensor,
    roi_size: Sequence[int] | int,
    sw_batch_size: int,
    pretrained_predictor: Callable[..., torch.Tensor | Sequence[torch.Tensor] | dict[Any, torch.Tensor]],
    ...
    **kwargs: Any,
    ...
    """
    # registering forward hooks again
    for layer in layers:
        getattr(pretrained_predictor,layer).register_forward_hook(get_features(layer))
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        pretrained_seg_prob_out = extract_features(win_data,layers,pretrained_predictor)
        # for feature in features:
        #     print(feature) # need to check that this is correct.
        seg_prob_out = predictor(win_data,pretrained_seg_prob_out, *args, **kwargs)  # batched patch
